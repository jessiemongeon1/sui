...
doc_goal:
  audience: Developers who need test tokens to start transacting on Sui Devnet or Testnet
  outcome: Can request and verify SUI token balance on a test network
  success_criteria:
    - Reader can obtain test SUI and confirm balance via CLI in under 2 minutes
    - Faucet rate limits and cooldowns are documented
    - At least 2 methods for requesting tokens (CLI, web) are shown
  non_goals:
    - Purchasing Mainnet SUI
