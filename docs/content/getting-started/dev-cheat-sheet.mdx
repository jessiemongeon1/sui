...
doc_goal:
  audience: Sui developers who need a quick reference for best practices across Move, apps, and security
  outcome: Can identify and apply at least one best practice per category (Move, testing, signing, zkLogin) without consulting other docs
  success_criteria:
    - Each best practice is actionable in one sentence or a short code snippet
    - Practices are grouped by domain with 100% of categories scannable in under 2 minutes
    - Anti-patterns are called out alongside the recommended approach
  non_goals:
    - In-depth tutorials for any single practice
    - Exhaustive Move language reference
---
