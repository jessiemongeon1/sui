...
doc_goal:
  audience: Ethereum/Solidity developers exploring Sui for the first time
  outcome: Can map core Solidity concepts (contracts, storage, inheritance, access control) to their Sui/Move equivalents
  success_criteria:
    - Every comparison row in the table links to a deeper resource for the Sui side
    - Reader can identify the Move equivalent for at least 5 key Solidity patterns
    - Code examples compile and run against the current Sui framework version
  non_goals:
    - Complete Move language tutorial
    - Solidity-to-Move code migration tooling
---
