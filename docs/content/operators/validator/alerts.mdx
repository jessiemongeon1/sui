...
:::caution
The `tx_type` labels `shared_certificate` and `owned_certificate` are no longer emitted. They have been replaced by `_deprecated_certificate`. The label `_deprecated_user_transaction` is now emitted for v1 `UserTransaction` types. Update any existing alert rules or dashboards that reference the old label names. Alerts using the old label names will produce no matches and will silently stop firing.
:::
...
sum(
  ...
  and
  histogram_quantile(0.95, sum by (le, host) (
    rate(sequencing_certificate_latency_bucket{network="$network", position="0", tx_type=~"_deprecated_certificate|soft_bundle"}[2m])
  )) < 2
) < 5000
```
