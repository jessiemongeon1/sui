...
- [x] A [Testnet SUI balance](/getting-started/onboarding/get-coins) deposited into the sponsor address's balance.
...
// Runs the default validation policy; add validators like
// gasBudget({ max }) or allowedPackages([...]) to tighten it.
...
In the app, build the transaction with the sponsor as gas owner and have the user sign the final bytes. Then send the bytes and user signature to your server, where the sponsor validates, co-signs, and executes:

```typescript
// Client side: the sponsor pays from its address balance (no specific gas coins).
tx.setSender(senderAddress);
...
Display the user's past payments by querying transactions sent by their address, using the same `SuiGrpcClient` as the rest of the app.
