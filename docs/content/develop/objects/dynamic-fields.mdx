...
In addition to `borrow` and `borrow_mut`, the `sui::dynamic_field` (and `sui::dynamic_object_field`) modules provide several higher-level access functions:

- **`borrow_or_add`**: Returns an immutable reference to the field value at `name`, inserting `default` first if no such field exists. This avoids the abort that `borrow` would cause on a missing field.
- **`borrow_mut_or_add`**: Returns a mutable reference to the field value at `name`, inserting `default` first if no such field exists. This avoids the abort that `borrow_mut` would cause on a missing field.
- **`get_do`**: If a field at `name` exists, borrows it immutably and calls the provided function on it; otherwise does nothing.
- **`get_mut_do`**: If a field at `name` exists, borrows it mutably and calls the provided function on it; otherwise does nothing.
- **`get_fold`**: Takes an initial accumulator value and, if a field at `name` exists, passes both the field (borrowed immutably) and the accumulator to a function returning a result; otherwise returns the initial value unchanged.
- **`get_mut_fold`**: Like `get_fold` but borrows the field mutably.

These functions are useful for avoiding explicit existence checks and simplifying patterns where a default value or a no-op is appropriate when a field is absent.

:::info

`sui::dynamic_object_field` has equivalent versions of all these convenience functions, consistent with its equivalents for `borrow`, `borrow_mut`, and `remove`.
