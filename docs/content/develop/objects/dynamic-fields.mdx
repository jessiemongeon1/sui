...
A transaction fails if it attempts to borrow a field that does not exist. Use `borrow_or_add` or `borrow_mut_or_add` to avoid this failure by inserting a default value when the field does not yet exist.
...
### Borrowing with a default value

If you want to borrow a field but are not sure whether it already exists, use `borrow_or_add` and `borrow_mut_or_add`. These functions return a mutable reference to the field, inserting it with the provided default value if the field does not yet exist — avoiding the abort behavior described above.
...
Both functions take a mutable reference to the object's `UID`, the field `name`, and a default `value`. If the field already exists, the default value is ignored and a reference to the existing field is returned. If the field does not exist, it is first added with the default value and then a reference to it is returned.

## Functional operations on dynamic fields

In addition to direct borrowing, the framework provides higher-order functions that accept a closure to operate on a field's value without requiring a separate borrow step.

### `get_do` and `get_mut_do`

`get_do` calls a function on the field's value if the field exists, returning an `Option` containing the function's return value. `get_mut_do` is the mutable variant.
...
If a field with the given `name` exists on `object`, the provided function `f` is called with a (mutable) reference to the field's value and its return value is wrapped in `option::some`. If the field does not exist, `option::none` is returned and `f` is not called.

### `get_fold` and `get_mut_fold`

`get_fold` and `get_mut_fold` fold an initial accumulator value with the field's value using a provided function, if the field exists.
...
These functions take an initial value `init` of type `A` and a function `f` that combines `A` with a (mutable) reference to the field's value to produce a new `A`. If the field exists, `f` is called and the result is returned. If the field does not exist, `init` is returned unchanged.
