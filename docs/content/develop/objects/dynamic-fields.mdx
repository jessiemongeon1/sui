...
:::note Checking existence and optional removal

Use `sui::dynamic_field::exists` to check whether a field with a given name and value type exists on an object before accessing or removing it. To remove a field without aborting if it doesn't exist, use `sui::dynamic_field::remove_opt`, which returns `Option<Value>`.

:::caution Deprecated functions
The previously used `exists_` and `remove_if_exists` functions are deprecated in favor of `exists` and `remove_opt`, respectively.
:::

`sui::dynamic_object_field` provides the same `exists` and `remove_opt` functions for object fields.
