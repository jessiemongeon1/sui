...
A transaction fails if it attempts to borrow a field that does not exist. Use `borrow_or_add` or `borrow_mut_or_add` to borrow a field and add it with a default value if it does not yet exist, avoiding this failure.
...
### Accessing or adding dynamic fields

The following functions borrow a field if it exists, or add it with a default value and return a reference if it does not:
...
`borrow_or_add` returns an immutable reference to the field's value, adding the field with `default` if it does not exist. `borrow_mut_or_add` returns a mutable reference, again adding the field with `default` if it is missing. Both functions avoid the abort that `borrow` and `borrow_mut` produce when a field is not found.

`sui::dynamic_object_field` has equivalent versions of both functions.

### Higher-order access functions

The following functions execute a closure on a field's value if the field exists:
...
`get_do` passes an immutable reference to the field's value to the closure `f` if the field exists, and does nothing otherwise. `get_mut_do` passes a mutable reference, allowing the closure to modify the field's value in place.

The following functions fold a value with the field's value if the field exists, returning the (possibly updated) accumulator:
...
`get_fold` passes an immutable reference to the field's value and the accumulator `val` to the closure `f` if the field exists, returning the result. If the field does not exist, `val` is returned unchanged. `get_mut_fold` works the same way but passes a mutable reference to the field's value, allowing the closure to modify it.

`sui::dynamic_object_field` has equivalent versions of all four higher-order access functions.
