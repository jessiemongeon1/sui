...
doc_goal:
  audience: Developers building their first Move package on Sui
  outcome: Can build, publish, and call a Move package on Testnet and inspect the resulting object
  success_criteria:
    - Reader can clone, build, publish, and call the example package end-to-end without errors
    - Core concepts (objects, packages, modules, Move.toml) are defined before first use
    - Every CLI command includes expected output or a way to verify success
  non_goals:
    - Move language deep dive beyond Hello World
    - Custom module authoring from scratch
---
...
<!-- [external GitHub content: MystenLabs/sui-stack-hello-world//move/hello-world/Move.toml — fetch at runtime] -->
...
<!-- [external GitHub content: MystenLabs/sui-stack-hello-world//move/hello-world/sources/greeting.move — fetch at runtime] -->
